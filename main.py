...
def main():
    ...
    if not file_path.lower().endswith(expected_extension):
        print(f"Программа: Ожидается {file_type}-файл с расширением {expected_extension}")
        return
    ...
    if get_yes_no_input("Программа: Отсортировать операции по дате? Да/Нет"):
        ascending = get_yes_no_input("Программа: Отсортировать по возрастанию или по убыванию? (да — возрастание, нет — убывание)")
    ...
    # Поиск по описанию
    if get_yes_no_input("Программа: Отфильтровать список транзакций по определённому слову в описании? Да/Нет"):
